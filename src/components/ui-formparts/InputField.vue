<template>
  <div :class="ignorePadding ? '' : dense ? 'q-pa-xs' : 'q-pa-sm'">
    ...
    <InputCpfCnpj
      :Icon="Icon"
      v-if="type == 'cpf' || type == 'cnpj' || type == 'cpf+cnpj'"
      :CpfOnly="type == 'cpf'"
      :hint="hint"
      :CnpjOnly="type == 'cnpj'"
      :dense="dense"
    ...
    <DatePicker
      ...
      :minDatePage="minDatePage"
      :maxDatePage="maxDatePage"
      v-model="value"
      :Default="Default"
      :Label="Label"
      :Error="Error"
...
export default {
  ...
  created() {
    this.inputRefId = `InputFieldRef-${this.$getService(
      "toolcase/utils",
    ).uniqid()}`;
