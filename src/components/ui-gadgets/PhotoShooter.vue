<template>
  <div :class="`row ${dense ? 'q-pa-xs' : 'q-pa-sm'}`">
    ...
    <div
      ...
      :class="['video-wrapper', 'col-12', 'bg-black']"
    >
      <video
        ref="video"
...
export default {
  ...
  props: {
    ...
    Orientation: {
      ...
      validator: (val) =>
        [null, "portrait", "landscape", "square"].includes(val),
    },
    AspectRatio: {
      type: String,
    ...
    async startCamera() {
      try {
        ...
        const constraints = {
          video: {
            facingMode: {
              ideal: this.DefaultCamera === "front" ? "user" : "environment",
            },
          },
        };
        ...
        if (String(err).includes("NotFoundError")) {
          this.$getService("toolcase/eventbroadcaster").$broadcast("no-camera");
        }
        console.error(
          "An error has occured on the attempt to start the camera",
          err,
        );
      }
    },
