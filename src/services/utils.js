...
export default {
  ...
  currentDatetime: () => {
    ...
    return new Date(
      UTC_now.toLocaleString("en", { timeZone: "America/Sao_Paulo" }),
    );
  },
  ...
  async waitFor(condition) {
    if (condition !== true) await this.waitFor(condition);
  ...
  notifyError: function (error) {
    const closeAction = { icon: "close", color: "white", round: true, handler: () => {} };
    if (
      error.response &&
      error.response.data &&
      (error.response.data.accessible || error.response.data.userReadable)
    ) {
      Notify.create({
      ...
      Notify.create({
        message:
          "Houve um problema para executar esta ação. Tente novamente mais tarde.",
        type: "negative",
        position: "top-right",
        actions: [closeAction],
      });
  ...
  async getAddressByZipCode(zipcode, notifyNotFound) {
    ...
    var response = await fetch(
      `https://viacep.com.br/ws/${encodeURIComponent(zipcode)}/json/`,
    );
  ...
  validatePasswordStrengh: function (obj) {
    var strongRegularExp = new RegExp(
      "^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])(?=.{8,})",
    );
    var mediumRegularExp = new RegExp(
  ...
  validateForm(input, inputError, customRules) {
    ...
    for (let k in input) {
      let field = input[k];
      if (
        (field === null || typeof field == "undefined" || field === "") &&
        k in inputError
      ) {
  ...
  dateFormat: function (date, format) {
    ...
    if (dateFormat.m < 10) dateFormat.m = "0" + dateFormat.m;
    if (dateFormat.d < 10) dateFormat.d = "0" + dateFormat.d;
    if (dateFormat.h < 10) dateFormat.h = "0" + dateFormat.h;
    if (dateFormat.i < 10) dateFormat.i = "0" + dateFormat.i;
    if (dateFormat.s < 10) dateFormat.s = "0" + dateFormat.s;
